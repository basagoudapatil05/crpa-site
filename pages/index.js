...
const demo = {
  ongoing:[{id:1,name:'Temple Project',location:'Dharwad',images:['/images/CR PATIL DR TEKKALAKI FIRST FLOOR RENDER BATCH 1 V4.jpg'],scope:'Institutional'}],
  upcoming:[{id:2,name:'Greenline Park',location:'Dharwad',images:['/images/1001.jpg'],scope:'Commercial'}],
  completed:[{id:3,name:'Premium Residence',location:'Belagavi',images:['/images/PHOTO-2025-08-05-12-55-50 (2).jpg'],scope:'Design + Build'}]
}
    ...
    <div>
      ...
      <main className="container">
        <section className="hero">
          ...
          <div style={{width:420}}>
            <div className="card"><img src="/images/1001.jpg" style={{width:'100%',borderRadius:10}}/></div>
            <div style={{marginTop:8,textAlign:'right'}} className="card"><strong>1100+</strong> Projects Delivered</div>
          </div>
        </section>
              ...
              {demo[section].map(p=>(
                <div className="project-card" key={p.id}>
                  ...
                  <div className="project-meta">
                    ...
                    <div style={{fontSize:12,letterSpacing:1,color:'var(--muted)',marginTop:6}}>{section.toUpperCase()}</div>
                    <div style={{fontSize:13,color:'var(--muted)',marginTop:8}}>{p.location} • {p.scope}</div>
                    <div style={{display:'flex',gap:8,marginTop:10}}>
                      {p.images.map((img,i)=>(
                        <button key={i} onClick={()=>setLight(img)} style={{width:44,height:44,borderRadius:6,overflow:'hidden',border:'1px solid rgba(255,255,255,0.06)'}}>
      ...
      {light && (
        <div style={{position:'fixed',inset:0,display:'grid',placeItems:'center',background:'rgba(0,0,0,0.8)'}} onClick={()=>setLight(null)}>
          <div style={{maxWidth:900}}><img src={light} style={{width:'100%',borderRadius:10}}/></div>
        </div>
      )}
    </div>
  )
